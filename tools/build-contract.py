...
S['Evidence']=obj({'id':id_schema,'claim':string(),'kind':enum('declaration','certification'),'status':enum('declared','unverified','verified','expired','demo'),'organization':nullable(string()),'referenceUrl':nullable(string(format='uri')),'validFrom':nullable(date),'validUntil':nullable(date),'checkedAt':nullable(date),'sourceId':id_schema},description='verified nécessite organisme, référence vérifiable et date de vérification; validité contrôlée à la lecture. Une déclaration ne devient jamais une certification.')
...
'/api/v1/accommodations':{'get':op('accommodations','AccommodationsResponse',[param('destinationId',id_schema,True),param('bicycleParking',enum('true','false'),description='Omis: aucun filtre; true/false excluent tous deux les valeurs inconnues.'),param('publicTransportNearby',enum('true','false')),*page])},
'/api/v1/methodology':{'get':op('methodology','Methodology')},
}
...
'accommodations':{'status':'complete','items':[{'id':'demo-stay','name':'Hébergement fictif de contrat','destinationId':'demo-lyon','dataStatus':'demo','features':{'bicycleParking':True,'publicTransportNearby':None},'publicTransportDistanceMeters':None,'price':None,'evidence':[],'provenance':provenance}],'page':{'limit':20,'offset':0,'total':1},'sources':[source],'warnings':['Aucune certification, prix ou disponibilité annoncés.']},
'methodology':{'version':'foundation-v1','status':'unavailable','indicator':'kgCO2e','scopes':[],'rules':['Données inconnues représentées par null, jamais zéro.','Pas de comparaison de périmètres incompatibles.'],'limits':['Aucun facteur réel vérifié au stade socle.','Pas de bilan hôtelier ni de score écologique global.'],'sources':[]},
...
contract={'openapi':'3.0.3','info':{'title':'Ecotrip internal API','version':'1.0.0','description':'Contrat MVP. Seul /health fonctionne en TASK-0001; endpoints métier planifiés, non simulés par le serveur. Les exemples sont synthétiques et hors ligne. Même origine, pas de compte ni réservation.'},'servers':[{'url':'/'}],'paths':paths,'components':{'schemas':S}}
