...
- Sélection : identifiant `28000` seulement; valeur publiée `0,151 kgCO2e/passager.km`; source amont « UTP - Enquête TCU 2017 ». Les lignes `Poste` officielles épinglées — `Carburant (amont/combustion) = 0,129` et `Fabrication = 0,0225` — sont exigées avec la ligne totale et qualifient le périmètre `life_cycle`.
...
L’import est local, transactionnel, sérialisé et épinglé par checksum; aucune recherche ne contacte ADEME. Le registre qualifié actuel n’accepte que V23.6, son checksum officiel et la date effective `2026-06-30`. Le schéma conserve structurellement l’historique, mais une publication ultérieure doit faire l’objet d’un nouveau mapping qualifié dans le code avant de pouvoir être importée; il ne faut ni écraser ni supprimer V23.6. La provenance de publication est figée dans chaque ligne `emission_factor_import`, et non lue depuis la ligne mutable `data_source`. Voir `docs/methodology.md` et le README. La fréquence de contrôle suit chaque publication ADEME (irrégulière).
