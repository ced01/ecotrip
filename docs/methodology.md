...
L’identité historique inclut source, identifiant externe et version de publication. Une réimportation au même checksum renvoie l’import existant sans mutation uniquement après requalification de son intégrité persistée complète; toute corruption fait échouer la réimportation. L’import courant refuse toute version autre que V23.6 et tout checksum autre que le snapshot officiel. Une même version avec un checksum différent, un doublon contradictoire, un identifiant attendu absent, une preuve `Poste` incomplète, un checksum/encodage/séparateur/colonnes/date/statut/unité/valeur/mapping invalide fait échouer toute la transaction. Un verrou transactionnel PostgreSQL sérialise deux imports concurrents de la même publication.

Le registre de publications du runtime est construit uniquement par la fabrique officielle sans paramètre et contient exactement V23.6 avec son checksum épinglé; c’est le seul registre éligible à l’activation du fournisseur `ademe`. La capacité structurelle multi-version est vérifiée en intégration avec des registres explicitement réservés aux tests (checksum arbitraire ou versions préfixées `SYNTHETIC-`), créés directement dans un schéma de test : l’ancienne ligne reste conservée et la date de voyage sélectionne déterministement la publication qualifiée la plus récente. Même structurellement complets, ces registres rendent toujours `isInitialized()` faux et ne peuvent jamais être activés par la factory. Ces entrées ne passent jamais par l’importeur, ne sont pas des publications ADEME revendiquées et démontrent l’historisation sans inventer une V23.7 réelle. À chaque lecture, import et facteur doivent correspondre exactement au registre, y compris l’identité déterministe du facteur (source/version/checksum/date effective, éditeur/URL/licence/date d’accès/méthode, valeur, dates et provenance métier complètes). Une publication enregistrée mais absente ou corrompue ferme la sélection et fait échouer une tentative de réimport idempotent au lieu de revenir à une version ou à un facteur démo.
